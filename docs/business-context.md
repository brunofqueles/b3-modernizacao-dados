...
- **Resposta a incidente real, não simulado**: um bug de cache no KNIME (ferramenta legada) travou o cálculo do índice por 6 dias sem sinal de erro visível — descoberto por investigação ativa (comparação de valores entre dias, não por acaso), com causa raiz identificada, corrigido, e o histórico de dados afetados tratado com honestidade (nem apagado, nem "consertado" com dado inventado). Ver [ADR-16](adr/adr-16-bug-cache-knime-janela-reconciliacao.md).
