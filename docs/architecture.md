...
**Dívida técnica:** `modo_execucao` inconsistente entre notebooks (só `01` e `02` têm Widget); 2 casos de duração anormal sem causa confirmada (01/09 e 03/09, ambos em `01_ingestao_landing`); limiar de duração do Job (5 min) precisa de mais amostras antes de recalibrar — hoje (03/09) uma execução com múltiplos programas locais abertos levou 14,36 min ao todo, disparando os 2 alertas nativos (Task e Job), mas tratado como possível causa externa não confirmada; mensagens dos alertas nativos permanecem no template padrão (decisão de escopo, não lacuna).
...
**Saúde operacional do pipeline (03/09):** 77 execuções registradas, 100% com sucesso, 0 falhas. 16 anomalias detectadas pela auditoria automática desde o início do projeto — 15 com causa raiz conhecida, 1 requerendo atenção (duração anormal em `01_ingestao_landing`, 01/09, sem causa confirmada). Um segundo caso de duração anormal foi detectado em 03/09 (`01_ingestao_landing`, 638,6s) — causa reportada pelo autor (múltiplos programas abertos na máquina local), mas não confirmada tecnicamente; tratado como amostra única, sem disparar recalibração de limiar. 0 gaps de execução esperada.
