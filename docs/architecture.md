...
| **Databricks Workflows (Job `pipeline_diario_b3`)** | Orquestra 6 Tasks em cadeia (Landing → Bronze → Silver → Gold → Reconciliação → Alerta de Divergência), agendado para dias úteis às 17h15, com notificação de falha e duração por e-mail. Completa a automação real do pipeline, sem intervenção manual. Ver [ADR-06](adr/adr-06-orquestracao-workflows-yaml.md), [ADR-10](adr/adr-10-reconciliacao-d1-automatizada.md), [ADR-12](adr/adr-12-alertas-nativos-falha-duracao.md), [ADR-13](adr/adr-13-tryexcept-completo-alerta-divergencia.md). |
...
| Job `pipeline_diario_b3` executando duas vezes, próximo uma da outra, em pelo menos 2 dias (28/08, 01/09) | Causa não investigada — sem impacto de dado (MERGE idempotente evita duplicação), mas gera consumo de compute e ruído no histórico de observabilidade. Ver [ADR-11](adr/adr-11-dashboard-genie-consumo-executivo.md). |
...
- **Observabilidade — refinamentos além do já implementado**: a tabela de log de execução (`observability.pipeline_runs`) está implementada e validada (ver [ADR-08](adr/adr-08-observabilidade-modulo-compartilhado.md)). Tratamento explícito de erro (`try/except`, registro de `status = "falha"` com `mensagem_erro`) agora cobre **todos os 5 notebooks** do pipeline — ver [ADR-13](adr/adr-13-tryexcept-completo-alerta-divergencia.md). Evoluções ainda pendentes: (1) propagação consistente de `modo_execucao` via Widget em todos os notebooks; (2) investigar a causa da execução dupla do Job observada em 28/08 e 01/09 (ver [ADR-11](adr/adr-11-dashboard-genie-consumo-executivo.md)); (3) painel de observabilidade do dashboard ainda não reflete "falhas por notebook" — dado real de falha só ficou disponível a partir do ADR-13.
...
- Job `pipeline_diario_b3`: **6 Tasks em cadeia** (Landing → Bronze → Silver → Gold → Reconciliação → Alerta de Divergência), agendado dias úteis às 17h15, notificação de falha e duração por e-mail (2 endereços, propósitos distintos). YAML reexportado após a 6ª Task.
...
**Padrão de desenvolvimento adotado:** PySpark como linguagem primária. Funções compartilhadas via `%run`. Exceções documentadas: datasets do AI/BI Dashboard em SQL (única forma nativa nessa ferramenta — ver [ADR-11](adr/adr-11-dashboard-genie-consumo-executivo.md)); envio de e-mail via `smtplib`/`email.mime` (sem alternativa nativa em PySpark para essa finalidade).

**Dívida técnica:** `modo_execucao` inconsistente entre notebooks (só `01` e `02` têm Widget); causa da execução dupla do Job em 28/08 e 01/09 não investigada; limiar de duração do Job (5 min) pode precisar de recalibração após observação de execuções agendadas reais; painel de observabilidade do dashboard ainda não mostra falhas por notebook; mensagens dos alertas nativos permanecem no template padrão (decisão de escopo, não lacuna).

**Código já implementado (resumo):** `00_setup_catalog`, `01_utilitarios_pipeline` (funções compartilhadas), `01_ingestao_landing`, `02_bronze`, `03_silver`, `04_gold` (5 indicadores, incluindo índice acumulado — ver ADR-09), `05_reconciliacao` (D-1 automático — ver ADR-10), `06_alerta_divergencia` (aditivo — ver ADR-13), `pipeline_diario_b3.yml`, AI/BI Dashboard e Genie Space (ver ADR-11). Todos os 5 notebooks do pipeline com try/except completo (ver ADR-13).
...
**Próximo passo real:** painel de observabilidade com falhas por notebook; recalibração do limiar de duração do Job (após observação); investigação da execução dupla do Job; diagrama de arquitetura final; avaliar paralelização/lote das chamadas de API na ingestão antes de qualquer escala real (gargalo identificado na simulação de FinOps — ver [ADR-14](adr/adr-14-finops-armazenamento-dbu.md)).