...
- **A causa raiz do caso de duração anormal de 01/09** (`01_ingestao_landing`, 157,87s) — o único, entre as anomalias registradas, sem nenhuma explicação (foi confirmada por motivo de lentidão do PC devido em execução manual).
...
**Próximo passo real:** avaliar paralelização/lote das chamadas de API na ingestão antes de qualquer escala real (gargalo identificado na simulação de FinOps — ver [ADR-14](adr/adr-14-finops-armazenamento-dbu.md)); acumular mais amostras antes de decidir sobre recalibração do limiar de duração.