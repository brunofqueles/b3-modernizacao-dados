...
- Job `pipeline_diario_b3`: **7 Tasks em cadeia** (Landing → Bronze → Silver → Gold → Reconciliação → Alerta de Divergência → Auditoria de Execuções), agendado dias úteis às 17h15, notificação de falha e duração por e-mail (2 endereços, propósitos distintos). Todas as 7 Tasks com Parameter `modo_execucao=agendado` configurado explicitamente (corrigido em 04/09 — ver [ADR-18](adr/adr-18-correcao-modo-execucao-categorizacao-anomalias.md); `bronze` nunca teve isso configurado antes, funcionava por coincidência do valor padrão do Widget). YAML reexportado após a correção. Tempo de execução observado variando de ~3,4 min a 14,36 min conforme condições da execução (ver riscos, seção 6) — o limiar de 5 min já foi ultrapassado em pelo menos uma execução manual, disparando os alertas corretamente.
...
- Execuções reais do Job agendado (27/08, 28/08, 31/08, 01/09, 02/09, 03/09, 04/09, 17h15): sem intervenção manual.
...
**Dívida técnica:** `06_alerta_divergencia` **não tem** try/except nem chamada a `registrar_execucao` — diferente dos demais 6 notebooks do pipeline, uma falha nele não fica registrada em `observability.pipeline_runs` nem seria detectada pela auditoria automática (ADR-15); registrado como limitação conhecida, não corrigido nesta fase final. 2 casos de duração anormal sem causa confirmada (01/09 e 03/09, ambos em `01_ingestao_landing`, classificados como `NAO DETERMINADA`); limiar de duração do Job (5 min) precisa de mais amostras antes de recalibrar; mensagens dos alertas nativos permanecem no template padrão (decisão de escopo, não lacuna); `modo_execucao` não distingue com precisão o gatilho real da execução (agendado vs. manual via Job), apenas o que está configurado como Parameter fixo da Task — avaliado e aceito como suficiente (ver ADR-18).

**Código já implementado (resumo):** `00_setup_catalog`, `01_utilitarios_pipeline` (funções compartilhadas), `01_ingestao_landing`, `02_bronze`, `03_silver`, `04_gold` (5 indicadores, incluindo índice acumulado — ver ADR-09), `05_reconciliacao` (D-1 automático — ver ADR-10), `06_alerta_divergencia` (aditivo — ver ADR-13), `07_auditoria_execucoes` (detecção automática de anomalias — ver ADR-15), `08_investigacao_anomalias` (preenchimento manual centralizado de causa raiz — ver ADR-18), `pipeline_diario_b3.yml`, AI/BI Dashboard e Genie Space (ver ADR-11). Todos os 5 notebooks do pipeline core com try/except completo (ver ADR-13).
...
**Saúde operacional do pipeline (04/09):** 77+ execuções registradas, 100% com sucesso, 0 falhas (números seguem crescendo a cada execução agendada). 19 anomalias detectadas pela auditoria automática desde o início do projeto, agora categorizadas por origem (ver [ADR-18](adr/adr-18-correcao-modo-execucao-categorizacao-anomalias.md)): 12 `[TESTE]` (originadas do próprio desenvolvimento/teste do projeto), 5 `[OPERACIONAL]` (comportamento real — sincronização tardia do KNIME em 28/08), 2 `NAO DETERMINADA` (causa não confirmada, 01/09 e 03/09), 0 aguardando investigação, 0 gaps de execução esperada. Números exatos consultáveis a qualquer momento na página "Observabilidade Técnica" do Dashboard, que se atualiza automaticamente.
