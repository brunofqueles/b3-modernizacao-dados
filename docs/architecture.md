...
| Ausência de CSV do KNIME (feriado, dia sem execução manual) derrubando `alerta_divergencia` e `auditoria_execucoes` em cascata, mesmo com Landing/Bronze/Silver/Gold funcionando | **Descoberto em produção real (07/09, 09/09) e corrigido em 10/09** — reconciliação passa a registrar `status = pendente_knime` em vez de propagar erro; `06_alerta_divergencia` checa esse status antes de avaliar, evitando reavaliar/reenviar alerta sobre dado antigo. Ver [ADR-19](adr/adr-19-resiliencia-reconciliacao-csv-ausente.md). |
| Gap de execução em 08/09/2026 — nenhum registro de início nem fim do Job nesse dia útil, sem feriado | **Causa não confirmada** — agendamento verificado ativo antes e depois, sem evidência de log de infraestrutura disponível na Free Edition para diagnosticar. Registrado como observação em monitoramento, não como resolvido. Ver [ADR-19](adr/adr-19-resiliencia-reconciliacao-csv-ausente.md). |
...
- [ADR-18 — Correção de modo_execucao e categorização de anomalias (Teste vs. Operacional)](adr/adr-18-correcao-modo-execucao-categorizacao-anomalias.md)
- [ADR-19 — Resiliência a CSV ausente do KNIME: reconciliação não deve derrubar o pipeline](adr/adr-19-resiliencia-reconciliacao-csv-ausente.md)
...
- Workflow KNIME completo e funcional, 8 execuções reais (26/08, 27/08, 28/08, 31/08, 01/09, 02/09, 03/09, 04/09) — as 5 primeiras afetadas por um bug de cache não percebido na hora (ver [ADR-16](adr/adr-16-bug-cache-knime-janela-reconciliacao.md)), corrigido em 02/09; 02/09 em diante já seguindo o novo processo (Reset all + Execute all).
...
- Job `pipeline_diario_b3`: **7 Tasks em cadeia** (Landing → Bronze → Silver → Gold → Reconciliação → Alerta de Divergência → Auditoria de Execuções), agendado dias úteis às 17h15, notificação de falha e duração por e-mail (2 endereços, propósitos distintos). Todas as 7 Tasks com Parameter `modo_execucao=agendado` configurado explicitamente (corrigido em 04/09 — ver [ADR-18](adr/adr-18-correcao-modo-execucao-categorizacao-anomalias.md); `bronze` nunca teve isso configurado antes, funcionava por coincidência do valor padrão do Widget). Reconciliação resiliente a CSV ausente do KNIME desde 10/09 — registra `status = pendente_knime` e não derruba as Tasks seguintes, em vez de propagar erro (ver [ADR-19](adr/adr-19-resiliencia-reconciliacao-csv-ausente.md)). YAML reexportado após cada correção. Tempo de execução observado variando de ~3,4 min a 14,36 min conforme condições da execução (ver riscos, seção 6) — o limiar de 5 min já foi ultrapassado em pelo menos uma execução manual, disparando os alertas corretamente.
- Notebook `databricks/auditoria/09_verificacao_pos_deploy`, para checagem ad-hoc de estado do pipeline (ex.: até que data cada camada tem dado disponível) — deliberadamente **não automatizado via Job**, sem lógica fixa e repetível ainda. Ver [ADR-19](adr/adr-19-resiliencia-reconciliacao-csv-ausente.md).
- Notebook `databricks/auditoria/08_investigacao_anomalias`, dedicado ao preenchimento manual de causa raiz (antes espalhado em células soltas em múltiplos notebooks), com taxonomia de prefixo `[TESTE]`/`[OPERACIONAL]` aplicada retroativamente às 19 anomalias já detectadas. Ver [ADR-18](adr/adr-18-correcao-modo-execucao-categorizacao-anomalias.md).
...
- Execuções reais do Job agendado (27/08, 28/08, 31/08, 01/09, 02/09, 03/09, 04/09, 07/09, 09/09, 17h15): sem intervenção manual — exceto 08/09, sem registro (ver dívida técnica).
...
**Dívida técnica:** 2 casos de duração anormal sem causa confirmada (01/09 e 03/09, ambos em `01_ingestao_landing`, classificados como `NAO DETERMINADA`); um terceiro caso recorrente de lentidão em `01_ingestao_landing` observado em 09/09 (621,71s), tratado como padrão a monitorar, não corrigido (comportamento consistente ao rodar manualmente, nunca acusou erro em execução agendada); limiar de duração do Job (5 min) precisa de mais amostras antes de recalibrar; mensagens dos alertas nativos permanecem no template padrão (decisão de escopo, não lacuna); `modo_execucao` não distingue com precisão o gatilho real da execução (agendado vs. manual via Job), apenas o que está configurado como Parameter fixo da Task — avaliado e aceito como suficiente (ver ADR-18); **gap de execução em 08/09/2026 sem causa confirmada** — nenhum registro de início nem fim do Job nesse dia, agendamento confirmado ativo antes e depois, sem evidência de log de infraestrutura disponível na Free Edition para diagnosticar (ver ADR-19).

**Código já implementado (resumo):** `00_setup_catalog`, `01_utilitarios_pipeline` (funções compartilhadas), `01_ingestao_landing`, `02_bronze`, `03_silver`, `04_gold` (5 indicadores, incluindo índice acumulado — ver ADR-09), `05_reconciliacao` (D-1 automático, resiliente a CSV ausente — ver ADR-10, ADR-19), `06_alerta_divergencia` (aditivo, com observabilidade completa desde 10/09 — ver ADR-13, ADR-19), `07_auditoria_execucoes` (detecção automática de anomalias — ver ADR-15), `08_investigacao_anomalias` (preenchimento manual centralizado de causa raiz — ver ADR-18), `09_verificacao_pos_deploy` (checagem ad-hoc, manual — ver ADR-19), `pipeline_diario_b3.yml`, AI/BI Dashboard e Genie Space (ver ADR-11). Todos os 7 notebooks do pipeline (core + aditivos) com try/except e observabilidade completos.
...
**Saúde operacional do pipeline (10/09):** execuções registradas majoritariamente com sucesso, exceto pelo gap não explicado de 08/09 e as reconciliações de 07/09 e 09/09, corretamente sinalizadas como `pendente_knime` (não falha) desde a correção do ADR-19. 19 anomalias detectadas pela auditoria automática entre o início do projeto e 04/09, categorizadas por origem (ver [ADR-18](adr/adr-18-correcao-modo-execucao-categorizacao-anomalias.md)): 12 `[TESTE]`, 5 `[OPERACIONAL]`, 2 `NAO DETERMINADA`. Números exatos, incluindo eventos após 04/09, consultáveis a qualquer momento na página "Observabilidade Técnica" do Dashboard, que se atualiza automaticamente.

**Este é o estado final planejado do projeto** — a partir de 04/09/2026, entra em fase de manutenção e validação, sem novo desenvolvimento programado, exceto em caso de falha real do pipeline. Resumo executivo de toda a jornada em [docs/entrega-projeto.md](entrega-projeto.md).

**Próximo passo real:** avaliar paralelização/lote das chamadas de API na ingestão antes de qualquer escala real (gargalo identificado na simulação de FinOps — ver [ADR-14](adr/adr-14-finops-armazenamento-dbu.md)); acumular mais amostras antes de decidir sobre recalibração do limiar de duração. Ambos são itens de retomada futura, não trabalho ativo — ver nota acima sobre o estado final do projeto.